...
import { combine, formatEpisode } from './utils/Helper.js';
...
const directoryPathInput = await input({ message: "[Folder Path] >> ", default: "/mnt/NAS/Harddisk/Download/meong" , required: true });
...
async function start(): Promise<void> {
    while (true) {
        ...
        for (const file of files) {
             ...
             * - [SubsMix] Yosuga no Sora - S01E01 (BD 1080p HEVC FLAC 2.0).mkv
            **/

            const match = file.name.match(/^\[([^\]]+)]\s+(.+?)\s+-\s+(.+?)(?:\s+\[|\.)/);
            if (!match) {
            ...
            const [, , animeName, episodeRaw] = match;

            // Extract episode number (handles 01, 01v2, S01E01, etc)
            const epMatch = episodeRaw.match(/(?:S\d+)?E?(\d+)/i);
            const episodeNumber = epMatch ? parseInt(epMatch[1]) : 0;

            parsed.push({
                originalFilename: file.name,
                seriesTitle: animeName.trim(),
        ...
        if (parsed.length === 0) {
            console.log(`[${tags.Error}] No valid anime files found in the directory.`);
            await input({
                message: "Enter anything to refresh."
            });
            continue;
        ...
        try {
            ...
            console.log(`\n[${tags.Jikan}] Season ${selectedSeason}`);
            ...
            for (let i = 0; i < jikanEntries.length; i++) {
                ...
                console.log(`[${tags.Jikan}] ${episodeStr} -> ${eps.title}`);
            }
